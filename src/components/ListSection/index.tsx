...
const INVENTORY_LIST_ID = "inventory-list";
...
export default function ListSection() {
  ...
  const estoqueList: List = {
    id: INVENTORY_LIST_ID,
    name: "Estoque",
    description: "Lista gerada automaticamente",
    resetAt: "",
    isRemoved: false,
    userId: "",
    itemId: [],
        ...
        >
          <div className="flex gap-3 overflow-x-auto snap-x snap-mandatory scrollbar-hide pb-2 px-4 scroll-smooth">
            ...
            {lists.map((list, index) => (
              <div
                key={index}
                className="relative rounded-2xl flex flex-col h-28 w-[150px] flex-shrink-0 shadow-sm hover:shadow-lg transition-all duration-300 snap-start overflow-hidden group bg-gradient-to-r from-purple-50 to-pink-50 border border-purple-100"
              >
                <div
                  className="flex-1 flex flex-col justify-center items-center p-4 cursor-pointer hover:bg-blue/20 active:scale-95 transition-all"
                  onClick={() => redirectToListPage(list.id)}
                >
                  <p className="text-slate-800 font-semibold text-center relative pb-1.5 group-hover:scale-105 group-hover:text-blue-600 transition-all duration-300 line-clamp-2">
                    {list.name}
                    <span className="absolute bottom-0 left-0 w-full h-0.5 bg-blue/40 rounded-full group-hover:h-1 transition-all duration-300"></span>
                  </p>
                </div>

                <button
                  onClick={(e) => handleShareClick(e, list)}
                  className={`flex items-center justify-center gap-1.5 py-2 px-3 text-xs font-medium transition-all duration-200 cursor-pointer bg-blue hover:bg-blue/60 text-white`}
                >
                  <Share2 className="w-3.5 h-3.5" />
                  <span>Compartilhar</span>
                </button>
