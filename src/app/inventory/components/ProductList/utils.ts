...
export function getSortedCategories(products: Product[]): string[] {
...
export function getItemsToBuyCount(items: Product[]): number {
  return items.length
    ? items.filter((item) => getItemStatus(item) === "needs-shopping").length
    : 0;
}

export function getCategoryStats(items: Product[]) {
